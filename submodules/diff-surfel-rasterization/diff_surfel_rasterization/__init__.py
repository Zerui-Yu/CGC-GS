...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, depth, radii, pixels, out_observe, out_all_map, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw.dump")
                print("\nAn error occured in forward. Please forward snapshot_fw.dump for debugging.")
                raise ex
        else:
            num_rendered, color, depth, radii, pixels, out_observe, out_all_map, out_plane_depth, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
        ...
        return color, radii, depth, pixels, out_observe, out_all_map, out_plane_depth

    @staticmethod
    def backward(ctx, grad_out_color, grad_radii, grad_depth, _, grad_out_observe, grad_out_all_map, grad_out_plane_depth):
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            try:
                grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations, depth, gard_all_map = _C.rasterize_gaussians_backward(*args)
            except Exception as ex:
