	...
	const bool debug)
{
  ...
  torch::Tensor out_others = torch::full({1+1, H, W}, 0.0, float_opts);
  ...
  return std::make_tuple(rendered, out_color, out_others, radii, pixels, out_observe, out_all_map, out_plane_depth, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug)
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, depth, dL_dall_map);
}
