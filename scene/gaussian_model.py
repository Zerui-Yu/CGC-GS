...
class GaussianModel:
    ...
    def get_normal_2d(self, view_cam):
        """
        compute normal for each 2D Gaussian
        """

        rot_mat = self.get_rotation_matrix()  # [N, 3, 3]

        v1 = rot_mat[:, :, 0]  # [N, 3]
        v2 = rot_mat[:, :, 1]  # [N, 3]

        normal_global = torch.cross(v1, v2, dim=-1)  # [N, 3]

        normal_global = normal_global / (normal_global.norm(dim=-1, keepdim=True) + 1e-8)

        gaussian_to_cam = view_cam.camera_center - self._xyz  # [N, 3]
        neg_mask = (normal_global * gaussian_to_cam).sum(dim=-1) < 0.0
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, max_points=-1):
        ...
        if max_points != -1:
            ...
            if selected_indices.shape[0] > max_points:
                ...
                grads_subset = padded_grad[selected_indices]
                idx_sorted = torch.argsort(grads_subset, descending=True)
                chosen_indices = selected_indices[idx_sorted[:max_points]]
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, max_points=-1):
        # Extract points that satisfy the gradient condition
        ...
        if max_points != -1:
            ...
            if selected_indices.shape[0] > max_points:
                ...
                grads_subset = torch.norm(grads[selected_indices], dim=-1)
                idx_sorted = torch.argsort(grads_subset, descending=True)
                chosen_indices = selected_indices[idx_sorted[:max_points]]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, max_points=-1):
    ...
    def densify_and_scale_split(self, grad_threshold, min_opacity, scene_extent, max_screen_size, percent_dense, scale_factor, scene_mask, N=2, no_grad=False, max_points=-1):
        assert scale_factor > 0
        n_init_points = self.get_xyz.shape[0]

        scale_mask = torch.max(self.get_scaling, dim=1).values > percent_dense * scene_extent * scale_factor
        if max_screen_size:
        ...
        if max_points != -1:
            selected_indices = torch.where(selected_pts_mask)[0]
            if selected_indices.shape[0] > max_points:
                chosen_indices = selected_indices[torch.randperm(selected_indices.shape[0])[:max_points]]
                chosen_mask = torch.zeros_like(selected_pts_mask)
                chosen_mask[chosen_indices] = True
                selected_pts_mask = chosen_mask
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, pixels):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter], dim=-1, keepdim=True) * pixels[update_filter]
    ...
    def scale_all_opacity(self, factor=0.9995):
        #not used
        with torch.no_grad():
            current_opacity = self.get_opacity
            new_opacity = current_opacity * factor
            new_internal_opacity = self.inverse_opacity_activation(new_opacity)
            optimizable_tensors = self.replace_tensor_to_optimizer(new_internal_opacity, "opacity")
            self._opacity = optimizable_tensors["opacity"]